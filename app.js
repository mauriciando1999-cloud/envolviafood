...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (document.getElementById('onboardingForm')) initOnboarding();
...
// ==========================================
// FUNCIÓN GLOBAL: RENDERIZAR MENÚ LATERAL
// ==========================================
function renderSidebar() {
    const configData = localStorage.getItem('envolviafood_config');
            
    // Si no hay configuración y no estamos en el index, devolver al usuario al inicio
    if (!configData) {
    ...
    const activeModulesList = document.getElementById('activeModulesList'); // Este ID solo existe en el dashboard
    ...
    // Obtener la URL actual para saber qué botón resaltar
    const currentPath = window.location.pathname;

    for (const [key, isEnabled] of Object.entries(config)) {
        if (isEnabled && modules[key]) {
            // 1. Inyectar enlace en el Sidebar
            const link = document.createElement('a');
            link.href = modules[key].file; // Apunta al archivo correspondiente
            
            // Lógica para resaltar si estamos en esa página
            ...
            // 2. Si estamos en el Dashboard, rellenar los badges
...
document.addEventListener("DOMContentLoaded", () => {
    // Enrutador básico: Detecta la vista actual basándose en elementos clave del DOM
    if (document.getElementById('onboardingForm')) {
        initOnboarding();
    } else if (document.getElementById('sidebarMenu')) {
        initDashboard();
...
// ==========================================
// 1. LÓGICA DE ONBOARDING (index.html)
// ==========================================
function initOnboarding() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const config = {
            mod_interna: document.querySelector('input[name="mod_interna"]').checked,
            mod_costos: document.querySelector('input[name="mod_costos"]').checked,
            mod_delivery: document.querySelector('input[name="mod_delivery"]').checked,
            mod_mesas: document.querySelector('input[name="mod_mesas"]').checked,
            mod_cocina: document.querySelector('input[name="mod_cocina"]').checked
        };
...
function initDashboard() {
    ...
    if (configData) {
        ...
        const modules = {
            mod_interna: { title: "RRHH e Interna", icon: `<svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>` },
            mod_costos: { title: "Inventario y Costos", icon: `<svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>` },
            mod_delivery: { title: "Delivery", icon: `<svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>` },
            mod_mesas: { title: "Gestión de Mesas", icon: `<svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>` },
            mod_cocina: { title: "Cocina (KDS)", icon: `<svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.879 16.121A3 3 0 1012.015 11L11 14H9c0 .768.293 1.536.879 2.121z"></path></svg>` }
        };

        for (const [key, isEnabled] of Object.entries(config)) {
            if (isEnabled && modules[key]) {
                const link = document.createElement('a');
                // Si es el módulo de cocina, le damos un enlace real para probar la navegación
                link.href = key === 'mod_cocina' ? 'cocina.html' : `#${key}`;
                link.className = "flex items-center px-4 py-3 text-gray-300 hover:bg-gray-800 hover:text-white rounded-lg transition-colors";
                link.innerHTML = `${modules[key].icon} ${modules[key].title}`;
                sidebarMenu.appendChild(link);

                const badge = document.createElement('span');
                badge.className = "px-3 py-1 bg-green-100 text-green-800 text-sm font-medium rounded-full";
                badge.innerText = modules[key].title;
                activeModulesList.appendChild(badge);
            }
        }
...
// ==========================================
// 3. LÓGICA DE KDS COCINA (cocina.html)
// ==========================================
function initKDS() {
    let orders = [
        {
            id: 101, type: "Mesa 4", timestamp: Date.now() - 600000, status: "pending",
            items: [
                { name: "Hamburguesa Clásica", qty: 1, variations: ["- Sin cebolla", "+ Extra tocino"] },
                { name: "Papas Fritas Grandes", qty: 1, variations: [] }
            ]
        },
        {
            id: 102, type: "Delivery", timestamp: Date.now() - 120000, status: "pending",
            items: [
                { name: "Pizza Margarita", qty: 2, variations: ["- Masa fina"] },
                { name: "Refresco 1.5L", qty: 1, variations: [] }
            ]
        }
    ];
    ...
    // Función global para marcar listo (accesible desde el HTML y la voz)
                ...
                <div class="mb-3 border-b border-gray-700 pb-2 last:border-0">
                    ...
                    ${item.variations.length > 0 ? `
                        <ul class="mt-1 text-sm space-y-1">
                            ${item.variations.map(v => `<li class="${v.startsWith('-') ? 'text-red-400 line-through decoration-red-400 decoration-2' : 'text-green-400 font-medium'}">${v}</li>`).join('')}
                        </ul>
                    ` : ''}
                </div>
            `).join('');
    ...
    // ==========================================
    // CONTROL DE VOZ AVANZADO ("Oído Chef")
    // ==========================================
    const btnMic = document.getElementById('btn-mic');
    const voiceStatus = document.getElementById('voice-status');
    let recognition;
    let isListeningIntentionally = false; // Controla si el usuario quiere que el sistema escuche
    ...
    // Función para buscar un pedido analizando la frase
    function findOrderFromCommand(command) {
        // 1. Intentar buscar por número de mesa (ej: "mesa 4")
        const mesaMatch = command.match(/mesa\s*(\d+)/);
        if (mesaMatch) {
            const mesaNum = mesaMatch[1];
            return orders.find(o => o.type.toLowerCase().includes(`mesa ${mesaNum}`) && o.status === 'pending');
        }
        
        // 2. Si no hay mesa, intentar buscar por ID del pedido (ej: "orden 102")
        const idMatch = command.match(/\d+/);
        if (idMatch) {
            const orderId = parseInt(idMatch[0]);
            return orders.find(o => o.id === orderId && o.status === 'pending');
        }
    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
        ...
        recognition.onstart = () => {
            btnMic.classList.add('bg-red-600', 'listening-pulse');
            btnMic.classList.remove('bg-gray-700');
        ...
        recognition.onresult = (event) => {
            ...
            // PALABRA DE ACTIVACIÓN: Solo actúa si escucha la palabra "chef"
            if (command.includes('chef')) {
                
                // Acción 1: Marcar como listo
                if (command.includes('lista') || command.includes('listo') || command.includes('marca')) {
                    const targetOrder = findOrderFromCommand(command);
                    if (targetOrder) {
                        window.markAsReady(targetOrder.id);
                    } else {
                        speak("No encontré ese pedido, chef. Repítelo por favor.");
                    }
                } 
                // Acción 2: Leer el pedido
                    ...
                    if (targetOrder) {
                        ...
                        targetOrder.items.forEach(i => {
                            ...
                            if (i.variations.length > 0) {
                                txt += `Atención: ${i.variations.join(', ')}. `;
                            }
                        });
                        speak(txt);
                    } else {
                        speak("No encontré ese pedido para leer, chef.");
                    }
                }
            }
        };
        ...
        // BUCLE CONTINUO: Si el navegador corta la escucha, pero el usuario la quiere activa, la reiniciamos.
        recognition.onend = () => {
            if (isListeningIntentionally) {
                ...
                btnMic.classList.remove('bg-red-600', 'listening-pulse');
                btnMic.classList.add('bg-gray-700');
                voiceStatus.innerText = "Asistente inactivo";
            }
        };
    }

    // Toggle de activación manual del micrófono
    btnMic.addEventListener('click', () => {
        if (isListeningIntentionally) {
            isListeningIntentionally = false;
            if(recognition) recognition.stop();
            speak("Asistente desactivado.");
        } else {
            isListeningIntentionally = true;
            if(recognition) recognition.start();
            speak("Oído, chef."); // Respuesta inmediata al activarlo
        }
    });
...
// ==========================================
// 4. LÓGICA DE MESAS (mesas.html)
// ==========================================
function initMesas() {
    // Estado de la vista
    let isEditMode = false;
    
    // Datos de prueba (En el futuro esto vendrá de Supabase)
    let tables = [
        { id: 1, number: "1", x: 50, y: 50, status: "free", orders: [] },
        { id: 2, number: "2", x: 200, y: 50, status: "occupied", 
          orders: [
              { item: "Pizza Pepperoni", qty: 1, price: 12.50 },
              { item: "Cerveza Artesanal", qty: 2, price: 4.00 }
          ] 
        },
        { id: 3, number: "3", x: 50, y: 180, status: "waiting", 
          orders: [
              { item: "Pasta Carbonara", qty: 2, price: 14.00 },
              { item: "Tiramisú", qty: 1, price: 6.00 }
          ] 
        },
    ...
    function renderTables() {
        ...
        tables.forEach(table => {
            if (table.status === 'free') freeCount++;
            else occCount++;
            ...
            // Eventos según el modo
            if (isEditMode) {
                makeDraggable(el, table);
            } else {
                el.addEventListener('click', () => openTableDetails(table));
            }
    ...
    // Lógica para Arrastrar y Soltar (Drag & Drop en Canvas)
    function makeDraggable(element, tableData) {
        ...
        function closeDragElement() {
            ...
            // Guardar nuevas coordenadas en el array
            tableData.x = parseInt(element.style.left);
            tableData.y = parseInt(element.style.top);
        }
    }
    ...
    function openTableDetails(table) {
        ...
        let html = `
            <div class="mb-6 flex items-center gap-2">
                <span class="px-3 py-1 text-xs font-bold rounded-full ${statusMap[table.status].bgClass} ${statusMap[table.status].colorClass} uppercase tracking-wider">
        ...
        if (table.orders.length === 0) {
            html += `<div class="text-center py-8 text-gray-400">
                <svg class="w-12 h-12 mx-auto mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
            ...
            table.orders.forEach(order => {
                ...
                html += `
                    <li class="flex justify-between items-start border-b border-gray-100 pb-2">
                        <div>
                            <p class="font-bold text-gray-800">${order.qty}x ${order.item}</p>
                            <p class="text-xs text-gray-500">$${order.price.toFixed(2)} c/u</p>
                        </div>
            ...
            html += `</ul>
                <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 flex justify-between items-center mt-auto">
    ...
    toggleEdit.addEventListener('change', (e) => {
        ...
        // Ocultar panel si estábamos editando
    ...
    btnAddTable.addEventListener('click', () => {
        ...
        tables.push({
            id: newId, number: newId.toString(), x: 100, y: 100, status: "free", orders: []
        });
        renderTables();
    });
...
// ==========================================
// 5. LÓGICA DE DELIVERY (delivery.html)
// ==========================================
function initDelivery() {
    
    // Zonas de Delivery Predefinidas (Cobranza por área/distancia)
    let zones = [
    ...
    let deliveryOrders = [
        { 
            id: 1045, customer: "Carlos Martínez", phone: "0414-1234567",
            address: "Av. Francisco de Miranda, Edif. Torre Europa, Piso 4",
            zoneId: "z1", subtotal: 24.00, status: "ready"
        },
        { 
            id: 1046, customer: "Ana Gómez", phone: "0424-9876543",
            address: "Calle París con Monterrey, Res. Las Lomas",
            zoneId: "z2", subtotal: 35.50, status: "pending"
        },
        { 
            id: 1047, customer: "Luis Pérez", phone: "0412-5554433",
            address: "Av. Urdaneta, Esquina de Ánimas, Local 2",
            zoneId: "z3", subtotal: 18.00, status: "en_route"
        }
    ];
            ...
            zonesListEl.innerHTML += `
                <li class="flex items-center justify-between border border-gray-200 rounded-lg p-3 hover:bg-gray-50 transition">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">${zone.distance}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <span class="font-black text-gray-900">$${zone.fee.toFixed(2)}</span>
                        <button class="text-gray-400 hover:text-gray-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></button>
                    </div>
                </li>
            `;
        });
            ...
            ordersListEl.innerHTML += `
                <tr class="hover:bg-gray-50 transition group">
                    <td class="px-6 py-4">
                        ...
                        <p class="text-xs font-semibold text-gray-700 mt-1">${order.customer}</p>
                    ...
                    <td class="px-6 py-4 max-w-xs">
                        ...
                        <select onchange="window.updateOrderZone(${order.id}, this.value)" class="mt-2 text-xs border border-gray-300 rounded px-2 py-1 bg-white text-gray-600 outline-none focus:border-blue-500 w-full">
                            <option value="">Seleccionar área...</option>
                            ${zoneOptions}
                        </select>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-xs text-gray-500">Comida: $${order.subtotal.toFixed(2)}</p>
                        <p class="text-xs text-gray-500 border-b border-gray-200 pb-1 mb-1">Envío: $${deliveryFee.toFixed(2)}</p>
                        <p class="font-bold text-gray-900">Total: $${total.toFixed(2)}</p>
                    </td>
                    <td class="px-6 py-4 text-center">
                        <span class="px-3 py-1 rounded-full text-xs font-bold ${statusConfig[order.status].class}">
                            ${statusConfig[order.status].text}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        ${order.status === 'ready' ? 
                            `<button onclick="window.dispatchOrder(${order.id})" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-xs font-bold transition">Despachar</button>` 
    ...
    // Funciones globales expuestas para los botones en el HTML inyectado
    window.updateOrderZone = function(orderId, newZoneId) {
        const order = deliveryOrders.find(o => o.id === orderId);
        if (order) {
            order.zoneId = newZoneId;
            renderOrders(); // Re-renderiza para recalcular el total automáticamente
        }
    };
...
// ==========================================
// 6. LÓGICA DE INVENTARIO Y COSTOS (costos.html)
// ==========================================
function initCostos() {
    
    // 1. Base de Datos Simulada de Inventario (Materia Prima)
    // El costo base está estandarizado por la unidad de compra principal.
    const inventory = {
    ...
    let recipes = [
        {
            ...
            isActive: true,
            ingredients: [
                { id: 'ing_01', qty: 0.200 }, // 200 gramos
                { id: 'ing_02', qty: 1 },     // 1 unidad
                { id: 'ing_03', qty: 0.040 }, // 40 gramos
                { id: 'ing_04', qty: 0.030 }  // 30 ml
            ]
        },
        {
            id: 'rec_02',
            name: 'Papas Trufadas Grandes',
            retailPrice: 6.50,
            isActive: true,
            ingredients: [
                { id: 'ing_05', qty: 0.350 }  // 350 gramos
            ]
        },
        {
            id: 'rec_03',
    ...
    // Función principal: Calcula el costo de los alimentos basado en la receta
    function calculateRecipeCost(recipeIngredients) {
        return recipeIngredients.reduce((total, item) => {
            ...
            return total + (inventoryItem.costPerUnit * item.qty);
        }, 0);
    }

    // Calcula el porcentaje de Food Cost
    function calculateFoodCostPercentage(cost, price) {
        if (price <= 0) return 0;
    ...
    function renderRecipesTable() {
        ...
        recipes.forEach(recipe => {
            ...
            const fcPercentage = calculateFoodCostPercentage(cost, recipe.retailPrice);
            
            // Lógica de semáforo para el FC% (Lo ideal en restaurantes es ~30%)
            let fcColorClass = 'text-green-600 bg-green-50';
            if (fcPercentage > 35) fcColorClass = 'text-red-600 bg-red-50';
            else if (fcPercentage > 32) fcColorClass = 'text-yellow-600 bg-yellow-50';

            const isActiveBadge = recipe.isActive 
                ? '<span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-[10px] font-black uppercase">En Menú</span>' 
                : '<span class="px-2 py-1 bg-gray-200 text-gray-600 rounded-full text-[10px] font-black uppercase">Oculto</span>';

            const tr = document.createElement('tr');
            tr.className = "hover:bg-gray-50 transition cursor-pointer group";
            tr.onclick = () => openRecipeEditor(recipe);
            
            tr.innerHTML = `
                <td class="px-4 py-3 font-bold text-gray-900 group-hover:text-blue-600 transition">${recipe.name}</td>
                ...
                <td class="px-4 py-3 text-center">
                    <span class="px-2 py-1 rounded font-bold text-xs ${fcColorClass}">${fcPercentage.toFixed(1)}%</span>
                </td>
    ...
    // Abre el panel lateral para editar
    ...
    // Actualiza las matemáticas de la ficha técnica en vivo
    function updateRecipeMath() {
        if (!currentEditingRecipe) return;

        const cost = calculateRecipeCost(currentEditingRecipe.ingredients);
        const targetFcInput = document.getElementById('target-fc').value;
        const targetFcDecimal = parseFloat(targetFcInput) / 100;
        
        // FÓRMULA PRO: Precio Sugerido = Costo / (Food Cost % Objetivo)
        let suggestedPrice = 0;
        if (targetFcDecimal > 0) {
            suggestedPrice = cost / targetFcDecimal;
        }
            ...
            listEl.innerHTML += `
                <li class="flex items-center justify-between border-b border-gray-100 pb-2">
                    ...
                    <div class="text-right">
                        ...
                        <button class="text-[10px] text-red-500 hover:text-red-700 font-bold uppercase tracking-wider">Remover</button>
                    </div>
                </li>
            `;
        });
    ...
    document.getElementById('target-fc').addEventListener('input', updateRecipeMath);
